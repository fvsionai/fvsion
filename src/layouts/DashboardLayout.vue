...
<template>
  <div class="flex height-control transition-colors">
    <NavDrawer v-model="open" />
    <div
      class="flex flex-grow flex-col bg-zinc-100 text-zinc-900 transition-all dark:bg-zinc-900 dark:text-zinc-100"
      :class="{ 'lg:pl-64': open }"
    >
      <AppTopBar v-model="open" />
      <div class="grow px-2 pb-2 pt-14 text-black dark:text-white">
        <main>
          <router-view />
        </main>
      </div>
